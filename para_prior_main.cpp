...
int main() {
    ...
    // file name
    std::string filename = "para_redraw_3.csv";
    ...
    for (i = 0; i < N_num; i++){
        ...
        time_sum = 0;
        for (j = 0; j < repeat; j++){
            result = model(T, N, &para_rejection_resampling, false, "para_rejection", redraw_num, nthread);
            ENP_sum += result[0];
            MSE_sum += result[1];
            time_sum += result[2];
        }
        ...
        time_sum = 0;
        for (j = 0; j < repeat; j++){
            result = model(T, N, &para_metropolis_resampling, false, "para_metropolis", redraw_num, nthread);
            ENP_sum += result[0];
            MSE_sum += result[1];
            time_sum += result[2];
        }
        ...
        time_sum = 0;
        for (j = 0; j < repeat; j++){
            result = model(T, N, &para_systematic_resampling, false, "para_systematic", redraw_num, nthread);
            ENP_sum += result[0];
            MSE_sum += result[1];
            time_sum += result[2];
        }
        ...
        time_sum = 0;
        for (j = 0; j < repeat; j++){
            result = model(T, N, &para_stratified_resampling, false, "para_stratified", redraw_num, nthread);
            ENP_sum += result[0];
            MSE_sum += result[1];
            time_sum += result[2];
        }
