...
std::vector<double> model(int T, int N, void (*resampling_func)(std::vector<double>&, std::vector<int>&, int, std::vector<std::default_random_engine>&), bool save, std::string file_name, int nthreads = 1, double variance_v = 1, double variance_n = 1){
    /*
    ...
    */
    ...
    double weight_sum, reciprocal_weight_sum, particle_sum, effective_particle_num_sum, weight_squared_sum;
    ...
    for(t = 1; t < T ; t++){
        ...
        #pragma omp parallel reduction(+:weight_sum) shared(num_particle_per_thread, x_P_update, P_weight, x_P, std_dev_v, std_dev_n, variance_v) private(ithread, i)
        {   
        ...
        #pragma omp parallel reduction(+:particle_sum) shared(x_P, x_P_update, sample_index, num_particle_per_thread, N, variance_v) private(i, ithread)
        {   
            ithread = omp_get_thread_num();
            std::default_random_engine& generator = generator_vec[ithread]; // Get the generator based on thread id
            for (i = num_particle_per_thread * ithread; i < num_particle_per_thread * (ithread + 1) && i < N; i++){
                x_P[i] = x_P_update[sample_index[i]] + sqrt(variance_v) * normal_dist(generator);
                particle_sum += x_P[i];
            }
        }
        #pragma omp barrier
