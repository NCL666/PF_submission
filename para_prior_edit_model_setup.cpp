...
std::vector<double> model(int T, int N , void (*resampling_func)(std::vector<double>&, std::vector<int>&, int, std::vector<std::default_random_engine>&), bool save, std::string file_name, int redraw_num, int nthreads = 1){
    /*
    ...
    */
    ...
    double weight_sum, reciprocal_weight_sum, effective_particle_num_sum, particle_sum, weight_squared_sum;
    ...
    double suggested_redraw, y_redraw;
    ...
    for(t = 1; t < T + 1; t++){
        ...
        #pragma omp parallel reduction(+:weight_sum) shared(num_particle_per_thread, x_P_update, P_weight, x_P, std_dev_v, std_dev_n, redraw_num, y) private(ithread, i, j, suggested_redraw, diff, new_diff, y_redraw)
        {   
        ...
        // accummulate the sum of the particle weights
        particle_sum = 0.0;
        ...
        effective_particle_num_sum += 1.0/weight_squared_sum;
    ...
    if (save == true){
        ...
        // file name
        std::string redraw_num_str = std::to_string(redraw_num);
        std::string filename = file_name + "_prior_" +  redraw_num_str + ".csv";
