...
std::vector<double> model(int T, int N, void (*resampling_func)(std::vector<double>&, std::vector<int>&, int, std::vector<std::default_random_engine>&), bool save, std::string file_name, int nthreads = 1, int redraw_num = 0, double EPN_threshold = 0.5, double rough_var = 1.0){
    /*
   ...
   // parallel setup
    ...
    double weight_sum, reciprocal_weight_sum, effective_particle_num_sum, particle_sum, weight_squared_sum, ESS;
    //initialise difference between predicted and actual
    double diff, new_diff;

    // parameters of process noise v_k
    double mean_v = 0;
    double std_dev_v = sqrt(2);
    ...
    // particle threshold
    double P_threshold = EPN_threshold * (double) N;
    // initialise the redraw
    double suggested_redraw;
    ...
    effective_particle_num_sum = 0.0;
    ...
    for(t = 1; t < T + 1; t++){
        ...
        #pragma omp parallel reduction(+:weight_sum) shared(num_particle_per_thread, x_P_update, P_weight, x_P, std_dev_v, std_dev_n, redraw_num, y) private(ithread, i, j, suggested_redraw, diff, new_diff)
        {   
            ...
            for (i = num_particle_per_thread * ithread; i < num_particle_per_thread * (ithread + 1) && i < N; i++){
                ...
                diff = abs(x_P_update[i] - x);
                for (j = 1; j < redraw_num + 1; j++){
                    suggested_redraw = 0.5 * x_P[i] + 25 * x_P[i]/(1 + pow(x_P[i],2)) + 8*cos(1.2*(t-1)) + std_dev_v * normal_dist(generator);
                    new_diff = abs(suggested_redraw - x);
                    if (diff > new_diff){
                        diff = new_diff;
                        x_P_update[i] = suggested_redraw;
                    }
                }
        ...
        #pragma omp parallel reduction(+:weight_squared_sum) shared(P_weight, reciprocal_weight_sum, N) private(i, ithread)
        {
            ithread = omp_get_thread_num();
            for (i = num_particle_per_thread * ithread; i < num_particle_per_thread * (ithread + 1) && i < N; i++){
                P_weight[i] = P_weight[i] * reciprocal_weight_sum;
                weight_squared_sum += P_weight[i]* P_weight[i];
        ...
        // conditioned resampling
        
        if (ESS < P_threshold){
            // Resampling step
            resampling_func(P_weight, sample_index, nthreads, generator_vec);
            // update x_P given sample_index
            #pragma omp parallel reduction(+:particle_sum) shared(x_P, x_P_update, sample_index, num_particle_per_thread, N, rough_var) private(i, ithread)
            {   
                ithread = omp_get_thread_num();
                std::default_random_engine& generator = generator_vec[ithread]; // Get the generator based on thread id
                for (i = num_particle_per_thread * ithread; i < num_particle_per_thread * (ithread + 1) && i < N; i++){
                    x_P[i] = x_P_update[sample_index[i]] + sqrt(rough_var) * normal_dist(generator);
                    particle_sum += x_P[i];
                }
            }
            #pragma omp barrier
        ...
        // sum up the effective number of particles
        effective_particle_num_sum += ESS;
    ...
    result[0] = effective_particle_num_sum/T;
    result[1] = MSE_sum/T;
    result[2] = duration;
